...
const MOBILE_KEY="streamdrive_mobile_v1";
const IS_MOBILE=/Android|iPhone|iPad|iPod|Mobile/i.test(navigator.userAgent)||window.matchMedia?.("(max-width: 800px)").matches;
...
function load(){
 const isSeriesPage=location.pathname.endsWith("/series.html")||location.pathname.endsWith("\\series.html")||location.pathname.endsWith("series.html");
 try{
  if(IS_MOBILE){
   // No celular não analisamos o catálogo desktop de vários MB.
   // A página de séries carrega seu catálogo diretamente dos arquivos JS.
   const compact=localStorage.getItem(MOBILE_KEY);
   db=normalizeData(compact?JSON.parse(compact):clone(defaultData));
  }else{
   db=normalizeData(JSON.parse(localStorage.getItem(KEY))||clone(defaultData));
  }
 }catch(e){db=normalizeData(clone(defaultData))}
...
function save(){
 try{
  if(IS_MOBILE){
   // Mantém apenas dados essenciais no armazenamento móvel; séries são recarregadas
   // dos arquivos da página de séries, evitando estourar a cota e travar o navegador.
   const compact={accounts:db.accounts,favorites:db.favorites,history:db.history,reactions:db.reactions,contents:db.contents.filter(c=>c.type!=="series")};
   localStorage.setItem(MOBILE_KEY,JSON.stringify(compact));
  }else{
...
function initStreamPlayer(){
 ...
 if(video){
  ...
  // O Chrome pode bloquear/bugar a composição de vídeo quando o player abre
  // automaticamente. O vídeo começa pelo botão Play ou pela interação do usuário.
  setPlaying(false);
 }
...
function cardMarkup(c){
 ...
 return `<article class="card category-preview-card" data-content-id="${esc(c.id)}" onmouseenter="startCardPreview(this)" onmouseleave="stopCardPreview(this)">
   <div class="card-media" onclick="openContentInfo('${esc(c.id)}')">
     <img loading="lazy" decoding="async" class="poster poster-clickable" src="${esc(poster)}" onerror="this.onerror=null;this.src=posterFallback()" alt="${esc(c.title)}" title="Clique para assistir">
     ${video}
...
function renderCategoryPage(category,type=""){
 ...
 const previewMedia=previewSrc
  ? `<video class="category-page-preview-video" src="${esc(previewSrc)}" poster="${esc(previewItem.poster||posterFallback())}" autoplay muted loop playsinline preload="metadata" onerror="this.style.display='none';this.nextElementSibling.style.display='block'"></video><img loading="lazy" decoding="async" class="category-page-preview-fallback" src="${esc(previewItem.poster||posterFallback())}" alt="Prévia de ${esc(previewItem.title)}">`
 ...
 panel.innerHTML=`
   ...
   <div class="inline-info-media">
     ${getCardPreviewVideo(c)
       ...
       : `<img loading="lazy" decoding="async" class="inline-info-preview" src="${esc(poster)}" alt="${esc(c.title)}" onerror="this.onerror=null;this.src=posterFallback()">`}
...
if(session&&session.role==="user"){
 ...
 if(!IS_MOBILE || location.pathname.endsWith("series.html")){ setTimeout(()=>{hydrateSeriesImages().catch(()=>{});},0); }
}else{
 renderLogin();
}
